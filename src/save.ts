import { download, loadBinFile, loadFile, toArrayBuffer } from './utils';
declare global {
    interface Window {
        save: Save;
    }
...
async function processSaveFile(saveData: Uint8Array, name: string): Promise<Save> {
    ...
    if (saveJSON.version > 0) {
        ...
        counter += 32;
        let newHash = await computeSha256(toArrayBuffer(saveData.buffer));
...
export function downloadSaveFile(loadMessage?: HTMLSpanElement): void {
    ...
    if (window.save.version > 0) {
        myData = appendData(myData, salt.buffer);
    }
